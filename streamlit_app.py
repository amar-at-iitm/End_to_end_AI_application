...
try:
    ...
    if "Close" in df.columns and "Datetime" in df.columns:
        ...
        st.warning(
            "Required columns ('Close', 'Datetime') not found in uploaded data!"
        )
    ...
    if "Close" in df.columns:
        if len(df) >= SEQ_LENGTH:
            ...
            st.warning(
                f"Need at least {SEQ_LENGTH} rows to predict! (Currently have {len(df)} rows)"
            )
    else:
...
def notify_developer(feedback_details="Prediction was not satisfactory."):
    ...
    try:
        with smtplib.SMTP_SSL("smtp.gmail.com", 465) as smtp:
            smtp.login(
                "abhijit912813@gmail.com", "cbmghskpqrozbyyy"
...
if (
    st.session_state.feedback == "satisfied"
    and not st.session_state.prediction_made
):
    ...
    confirm_retrain = st.checkbox(
        "⚡ I confirm I want to send this to the developer"
    )
    if confirm_retrain:
        if st.button("Send Feedback to Developer"):
            notify_developer(
                "User marked the prediction as unsatisfactory. Please review the model output."
            )
